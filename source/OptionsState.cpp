...
/*virtual*/ bool OptionsState::Input( void )
{
	...
	if (mousePos.IsWithinRectangle(SGD::Rectangle(SGD::Point(left_start + 475.0F, 150.0F), SGD::Size(64, 64))))
	{
		if (pInput->IsKeyReleased(SGD::Key::MouseLeft) == true)
			volumes[0] -= volOffset;
		m_nCursor = 0;
	}
	else if (mousePos.IsWithinRectangle(SGD::Rectangle(SGD::Point(left_start + 625.0F, 150.0F), SGD::Size(64, 64))))
	{
		if (pInput->IsKeyReleased(SGD::Key::MouseLeft) == true)
			volumes[0] += volOffset;
		m_nCursor = 0;
	...
	else if (mousePos.IsWithinRectangle(SGD::Rectangle(SGD::Point(left_start + 625.0F, 300.0F), SGD::Size(64, 64))))
	{
		if (pInput->IsKeyReleased(SGD::Key::MouseLeft) == true)
			volumes[1] += volOffset;
	...
	if (mousePos.IsWithinRectangle(SGD::Rectangle(SGD::Point(left_start + 50, 500.0F), SGD::Size(64, 32))))
	{
		if (pInput->IsKeyReleased(SGD::Key::MouseLeft) == true)
			Game::GetInstance()->RemoveState();
...
/*virtual*/ void OptionsState::Render( void )
{
	...
	//SGD::GraphicsManager::GetInstance()->DrawRectangle(SGD::Rectangle(SGD::Point(50.0f + 475, 400.0f), SGD::Size(192, 32)), {}, { 0, 255, 0 }, 2);
	...
	pFont->Draw("OPTIONS", { (width - (7 * 32 * 1.0f)) / 2, 25 }, 2.0f, { 255, 255, 255 });


	// Align text based on window width
	float left_start	= 100.0F;				// 50
	...
	pFont->Draw("Music", { left_start, 100.0F }, scale, { 255, 0, 0 });
	pFont->Draw("Volume", { left_start + 100.0F, 150.0F }, scale, { 255, 0, 0 });

	pFont->Draw("Sound Effects", { left_start, 250.0F }, scale, { 255, 0, 0 });
	pFont->Draw("Volume", { left_start + 100.0F, 300.0F }, scale, { 255, 0, 0 });
	pFont->Draw("Full Screen", { left_start + 100.0F, 400.0F }, scale, { 255, 0, 0 });

	pFont->Draw(" < ", { left_start + 400.0F, 150.0F }, 1.5f, { 255, 0, 0 });
	pFont->Draw(" > ", { left_start + 550.0F, 150.0F }, 1.5f, { 255, 0, 0 });

	pFont->Draw(" < ", { left_start + 400.0F, 300.0F }, 1.5f, { 255, 0, 0 });
	pFont->Draw(" > ", { left_start + 550.0F, 300.0F }, 1.5f, { 255, 0, 0 });

	pFont->Draw(" < ", { left_start + 400.0F, 400.0F }, 1.5f, { 255, 0, 0 });
	pFont->Draw(" > ", { left_start + 550.0F, 400.0F }, 1.5f, { 255, 0, 0 });
	...
	{
	case 0:
		pFont->Draw(volumes[0].str().c_str(), { left_start + 500.0F, 150.0F }, scale, { 255, 255, 255 });
		pFont->Draw(volumes[1].str().c_str(), { left_start + 500.0F, 300.0F }, scale, { 255, 0, 0 });

		if (m_bFullScreen == false)
			pFont->Draw("ON", { (width - (3 * 32 ) - 100), 400.0F }, scale, { 255, 0, 0 });
		else
			pFont->Draw("OFF", { (width - (3 * 32 ) - 100), 400.0F }, scale, { 255, 0, 0 });

		pFont->Draw("Back", { left_start, 500.0F }, 1.0f, { 255, 0, 0 });

		break;
	
	case 1:
		pFont->Draw(volumes[0].str().c_str(), { left_start + 500.0F, 150.0F }, scale, { 255, 0, 0 });
		pFont->Draw(volumes[1].str().c_str(), { left_start + 500.0F, 300.0F }, scale, { 255, 255, 255 });
	...
	case 2:
		...
		if (m_bFullScreen == false)
			pFont->Draw("ON", { (width - (3 * 32) - 100), 400.0F }, scale, { 255, 255, 255 });
		else
			pFont->Draw("OFF", { (width - (3 * 32) - 100), 400.0F }, scale, { 255, 255, 255 });
	...
	case 3:
		...
		pFont->Draw("Back", { left_start, 500.0F }, 1.0f, { 255, 255, 255 });
